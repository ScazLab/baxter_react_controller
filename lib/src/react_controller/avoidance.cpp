...
AvoidanceHandlerAbstract::AvoidanceHandlerAbstract(const BaxterChain &_chain,
                                                   ...
                                                   chain(_chain),
                                                   collisionPoints(_collisionPoints), verbosity(_verbosity)
{
...
AvoidanceHandlerTactile::AvoidanceHandlerTactile(const BaxterChain &_chain,
                                                 const std::vector<collisionPoint> &_collisionPoints,
                                                 const unsigned int _verbosity):
                                                 AvoidanceHandlerAbstract(_chain,_collisionPoints,_verbosity)
{
    type="tactile";
...
Eigen::MatrixXd AvoidanceHandlerTactile::getVLIM(const Eigen::MatrixXd &v_lim)
{
    ...
    for (size_t i=0; i<ctrlPointChains.size(); i++)
    {
        ...
        Eigen::VectorXd s=(J.transpose()*normal) * avoidingSpeed * collisionPoints[i].m; //project movement along the normal into joint velocity space and scale by default avoidingSpeed and m of skin (or PPS) activation
        ...
        if (verbosity>=2)
        {
            ...
            // printf("s = (J.transposed()*normal) * avoidingSpeed * collisionPoints[i].m \n (%s)T = (%s)T * %f * %f\n",s.toString(3,3).c_str(),(J.transposed()*normal).toString(3,3).c_str(),avoidingSpeed,collisionPoints[i].m);
        }
