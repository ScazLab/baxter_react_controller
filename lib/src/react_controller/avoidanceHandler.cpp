...
Eigen::MatrixXd AvoidanceHandlerTactile::getV_LIM(const Eigen::MatrixXd &v_lim)
{
    ...
    for (size_t i = 0; i < ctrlPointChains.size(); ++i)
    {
        ...
        //.submatrix(0,2,0,ctrlPointChains[i].getNrOfSegments()-1);
        Eigen::MatrixXd J = ctrlPointChains[i].GeoJacobian().block(0, 0, 3, ctrlPointChains[i].getNrOfJoints()-1);
        ...
        Eigen::VectorXd normal = ctrlPointChains[i].getH().col(2).block<3,1>(0,0);//.subEigen::VectorXd(0,2);
        ...
        Eigen::VectorXd s = (J.transpose()*normal) * avoidingSpeed * collPoints[i].m;

        s = s * -1.0; //we reverse the direction to obtain joint velocities that bring about avoidance
        // ROS_INFO("s * (-1) -> joint contributions toward avoidance: \n (%s) \n",s.toString(3,3).c_str());
        // cout << s;
        for (size_t j = 0; j < size_t(s.rows()); ++j)
        {
            // ROS_INFO("Joint: %d, s[j]: %f, limits before: Min: %f, Max: %f\n",j,s[j],V_LIM(j,0),V_LIM(j,1));
            if (s[j] >= 0.0) //joint contributes to avoidance, we will set the min velocity accordingly
            {
                ...
                //          " adjusting Min; limits after: Min: %f, Max: %f\n",V_LIM(j,0),V_LIM(j,1));
            }
